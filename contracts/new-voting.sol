...
contract Voting {
    ...
    mapping(bytes32 => bool) public nikAlreadyUsed;
    mapping(bytes32 => address) public nikToAddress;
    ...
    function registerUser(bytes32 _hashedNIK, bytes memory _signature) public {
        // Verifikasi wallet belum terdaftar
        require(!users[msg.sender].registered, "User already registered.");
        
        //Cek NIK belum dipake wallet lain
        require(!nikAlreadyUsed[_hashedNIK], "This NIK is already registered by another wallet.");
        ...
        // TAMBAHIN: Mark NIK udah dipake
        nikAlreadyUsed[_hashedNIK] = true;
        nikToAddress[_hashedNIK] = msg.sender;
    ...
    // Function baru buat ngecek NIK udah dipake atau belum
    function isNIKRegistered(bytes32 _hashedNIK) public view returns (bool) {
        return nikAlreadyUsed[_hashedNIK];
    }
    
    // Function buat ngecek wallet udah register atau belum
    function isUserRegistered(address _wallet) public view returns (bool) {
        return users[_wallet].registered;
    }
    ...
    // Fungsi untuk mengecek apakah user sudah terdaftar (registered + verified)
    function checkUserRegistered(address _wallet) public view returns (bool) {
        return users[_wallet].registered && isVerifiedWallet[_wallet];
    }
    ...
    function checkNIKRegistered(bytes32 _hashedNIK) public view returns (bool) {
        return nikAlreadyUsed[_hashedNIK];
    }

    // Fungsi untuk mendapatkan address dari NIK (kalau ada)
    function getAddressByNIK(bytes32 _hashedNIK) public view returns (address) {
        require(nikAlreadyUsed[_hashedNIK], "NIK is not registered.");
        return nikToAddress[_hashedNIK];
    }
